...
linked_systems:
  ...
  - "winflowz_site/convex/schema.ts"
  - "winflowz_app/lib/features/auth"
  - "shipflow_data/technical/platforms/lemonsqueezy.md"
depends_on:
  - artifact: "shipflow_data/technical/platforms/lemonsqueezy.md"
    required_status: "draft"
  - artifact: "shipflow_data/workflow/specs/winflowz-android-lifetime-deal-launch.md"
    required_status: "draft"
supersedes: []
evidence:
  - "WinFlowz App founder offers use internal offer ids winflowz_app/focus, winflowz_app/power, winflowz_app/control, and winflowz_app/command."
...
This document is the reusable contract for paid access activation in the WinFlowz suite. It explains how a payment becomes product access, what Lemon Squeezy owns, what Convex owns, and what still needs a separate device-activation ledger.

## Vocabulary

- Payment provider: external system that collects money and emits signed events. Current direct-sale provider: Lemon Squeezy.
- Offer id: internal checkout id such as `winflowz_app/focus`.
- Product id: canonical suite product id such as `winflowz_app`.
...
| --- | --- | --- | --- | --- |
| Focus | `winflowz_app/focus` | `winflowz_app` | `focus` | 1 active device |
| Power | `winflowz_app/power` | `winflowz_app` | `power` | 2 active devices |
| Control | `winflowz_app/control` | `winflowz_app` | `control` | 5 active devices |
| Command | `winflowz_app/command` | `winflowz_app` | `command` | 10 active devices |

The Lifetime Deal grants access to present and future released WinFlowz platforms under the selected plan. The plan limit is the active-device count, not a per-platform SKU.
...
- Verify whether a `productEntitlements` row exists for `productId=winflowz_app`.
...
- Webhook route tests verify all four WinFlowz plans are forwarded to `bridge:processCommerceEvent`.

Hosted provider proof:

- Create a Lemon Squeezy test-mode checkout for a WinFlowz founder plan.
