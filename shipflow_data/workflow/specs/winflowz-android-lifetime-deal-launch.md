...
| 004-sf-deploy | partial | Release scope identified on 2026-06-18: payment activation docs, Founder webhook test coverage, the build-fix pair in `Pricing.astro` and `HeroSectionAlt.astro`, and a follow-up `docs.css` repair required by the first failed Vercel production build. Development mode is `hybrid` on Vercel, so live proof requires `005-sf-ship -> 405-sf-prod -> 108-sf-browser`. | Push the stylesheet repair, confirm the matching deployment URL, then run live founder-page proof. |
