...
owner: "Diane"
confidence: "high"
user_story: "En tant qu'utilisatrice du clavier Android WinFlowz, je veux que les touches puissent utiliser des gradients de matière qui suivent aussi l'effet d'appui choisi, afin que le clavier 2D ou 3D paraisse plus physique, plus premium et plus cohérent."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
depends_on:
  ...
  - artifact: "shipflow_data/workflow/specs/keyboard-physical-key-relief.md"
    artifact_version: "0.1.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/specs/keyboard-material-press-effects.md"
...
evidence:
  ...
  - "shipflow_data/workflow/specs/keyboard-material-press-effects.md already requires effect rendering through the key geometry, which now exposes a stable foundation for material gradients."
  - "shipflow_data/workflow/specs/keyboard-physical-key-relief.md already establishes the need for per-face material treatment in relief mode."
  ...
  - "android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/WinFlowzKeyboardView.kt already uses Canvas gradients and keyed geometry for relief/material rendering."
next_step: "/101-sf-ready shipflow_data/workflow/specs/keyboard-material-key-gradients.md"
---
...
Draft spec created on 2026-06-11 from Diane's request to add material gradients to key surfaces and make those gradients react coherently to the selected press effect. The next lifecycle step is `/101-sf-ready shipflow_data/workflow/specs/keyboard-material-key-gradients.md`.

## User Story

En tant qu'utilisatrice du clavier Android WinFlowz, je veux que les touches puissent utiliser des gradients de matière qui suivent aussi l'effet d'appui choisi, afin que le clavier 2D ou 3D paraisse plus physique, plus premium et plus cohérent.
...
- Manual checklist path: update or create `shipflow_data/workflow/test-checklists/keyboard-material-key-gradients.md`.
- Required scenarios:
...
- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardThemeModels.kt`: Kotlin schema and parsing.
- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/WinFlowzKeyboardView.kt`: native material geometry and gradient rendering.
- `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardStateStore.kt`: safe diagnostics/status exposure.
...
- `WinFlowzKeyboardView.kt` may become too dense if gradient resolution is embedded inline; extraction into a dedicated resolver/renderer helper is likely the professional path.
...
- Add or update a manual QA checklist at `shipflow_data/workflow/test-checklists/keyboard-material-key-gradients.md`.
...
- [ ] Tâche 7 : Strengthen validation and safe diagnostics
  - Fichiers : `lib/features/keyboard/domain/keyboard_theme_validation.dart`, `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardStateStore.kt`
...
- [ ] Tâche 8 : Update QA checklist and native documentation
  - Fichiers : `shipflow_data/workflow/test-checklists/keyboard-material-key-gradients.md`, `docs/technical/android-native.md`
...
- Extract native gradient resolution into a dedicated helper if `WinFlowzKeyboardView.kt` becomes harder to maintain.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-11 07:13:45 UTC | 100-sf-spec | GPT-5 Codex | Created a durable spec for material key gradients that also react to the selected press effect. | Draft spec written. | /101-sf-ready shipflow_data/workflow/specs/keyboard-material-key-gradients.md |
