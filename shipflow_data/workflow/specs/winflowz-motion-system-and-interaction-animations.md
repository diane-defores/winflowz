...
depends_on:
  ...
  - artifact: "shipflow_data/workflow/specs/winflowz-product-pages-ux-remaster.md"
    artifact_version: "1.0.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/specs/keyboard-physical-key-relief.md"
    artifact_version: "0.1.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/specs/keyboard-theme-studio.md"
    artifact_version: "0.1.0"
    required_status: "ready"
  - artifact: "shipflow_data/workflow/specs/onboarding-permissions-guide.md"
    artifact_version: "0.1.0"
    required_status: "ready"
supersedes: []
evidence:
...
next_step: "/sf-start shipflow_data/workflow/specs/winflowz-motion-system-and-interaction-animations.md"
---

# Spec: WinFlowz Motion System and Interaction Animations

🟢 [WinFlowzApp] spec: WinFlowz Motion System and Interaction Animations | status: ready | path: shipflow_data/workflow/specs/winflowz-motion-system-and-interaction-animations.md | next: /sf-start shipflow_data/workflow/specs/winflowz-motion-system-and-interaction-animations.md | id: wfz-motion-system

## Title

WinFlowz Motion System and Interaction Animations
...
En tant qu'utilisatrice WinFlowz Android et web, je veux que les animations des pages, composants et surfaces clavier rendent les interactions plus lisibles, physiques et professionnelles, afin de comprendre immédiatement ce qui change sans avoir l'impression que des effets sont simplement collés par-dessus l'interface.

## Minimal Behavior Contract

When the user taps, saves, syncs, changes page, edits content, opens a panel, presses a keyboard key, or completes an onboarding step, WinFlowz should use short, purposeful motion that visually belongs to the affected surface. The animation must communicate state, feedback, continuity, or physicality without changing the underlying data behavior, blocking the user, causing layout shifts, creating infinite motion, or hiding errors. If animation is disabled or not supported, the same state changes must remain understandable through static layout, labels, color, and status. The easy edge case to miss is the keyboard: glow, shake, relief, and special effects must transform the actual key surface/cube, not a detached decorative layer above it.
...
- No marketing-site animation work in `winflowz_site`.
- No animated illustration library, Lottie asset pipeline, or downloaded icon pack as the default path.
- No full redesign of the pages already covered by `winflowz-product-pages-ux-remaster.md`.
...
- Follow WinFlowz guardrails: local validation is limited to `flutter analyze`, `flutter test`, and targeted `flutter test ...`.
...
- Proof order:
  ...
  4. Web smoke proof: deployed Vercel Flutter web app at `https://app.winflowz.com/` on mobile common `390x844` plus desktop/narrow width.
  5. Manual proof: Diane validates professional feel, keyboard surface attachment, and absence of motion noise.
- Checklist path: no separate checklist artifact is required for this ready gate. If implementation touches native Android IME rendering, create `shipflow_data/workflow/test-checklists/winflowz-motion-system-and-interaction-animations.md` before device QA.
...
- Browser proof required before asking Diane for product-feel QA: Vercel Flutter web smoke on `https://app.winflowz.com/` at mobile common `390x844` and desktop/narrow web width.
...
- Reduced motion v1 relies on platform/media accessibility state and app-level motion gating in the shared primitives. Do not add a new user-facing WinFlowz reduced-motion setting in this chantier.
...
- `lib/core/theme/winflowz_theme_tokens.dart`: existing token layer with animation-related values.
...
- `winflowz-product-pages-ux-remaster.md` remains the structural source of truth for page layout; this spec layers motion on top of that structure.
...
- [ ] Task 1: Define app motion tokens.
  - File: `lib/core/theme/app_theme.dart` and/or `lib/core/theme/winflowz_theme_tokens.dart`
...
- [ ] Task 11: Run full validation and browser smoke.
  ...
  - Action: Run required Flutter checks, then verify Vercel Flutter web at `https://app.winflowz.com/` after deployment.
...
|----------|-------|-------|--------|--------|-----------|
| 2026-06-10 20:12:43 UTC | sf-spec | GPT-5 Codex | Created motion-system spec from user request and current Flutter UI context. | Draft saved. | `/sf-ready WinFlowz Motion System and Interaction Animations` |
| 2026-06-10 20:18:30 UTC | sf-ready | GPT-5 Codex | Reviewed readiness, resolved open motion-priority questions, tightened proof contract, and versioned dependent specs. | Ready. | `/sf-start shipflow_data/workflow/specs/winflowz-motion-system-and-interaction-animations.md` |
...
Next command: `/sf-start shipflow_data/workflow/specs/winflowz-motion-system-and-interaction-animations.md`
