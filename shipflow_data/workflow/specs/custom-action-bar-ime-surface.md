...
owner: "Diane"
confidence: high
user_story: "En tant qu'utilisatrice WinFlowz, je veux configurer une barre d'action unique et scrollable dans une page dédiée, puis l'activer dans le clavier Android IME, afin d'utiliser mes commandes personnalisées directement pendant la saisie."
risk_level: "high"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
  ...
  - "Sentry diagnostics"
depends_on:
  - artifact: "shipflow_data/workflow/specs/custom-action-buttons-and-command-macros.md"
    artifact_version: "1.0.0"
    required_status: "reviewed"
  - artifact: "shipflow_data/workflow/specs/android-ime-winflowz_app-keyboard.md"
    artifact_version: "unknown"
    required_status: "reviewed"
  - artifact: "shipflow_data/workflow/specs/keyboard-action-row-scroll-affordance.md"
    artifact_version: "unknown"
    required_status: "reviewed"
...
En tant qu'utilisatrice WinFlowz, je veux configurer une barre d'action unique
...
WinFlowz doit offrir une page dédiée aux actions qui permet de créer,
...
- Given elle active la barre depuis la page Actions ou Settings > Keyboard,
  when elle ouvre le clavier Android WinFlowz dans un champ standard, then une
...
- Respecter `shipflow_data/technical/design-system-authority.md`: nouveaux
  visuels Flutter via `WinFlowzThemeTokens`, `AppTheme`, `AppSpacing`,
...
- Proof order:
  ...
  3. ShipFlow metadata and design-system drift checks.
  4. GitHub Actions/Blacksmith Android/Kotlin checks.
  5. Diane physical-device IME QA.
- Checklist path: `shipflow_data/workflow/test-checklists/custom-action-bar-ime-surface.md`
...
- Existing Android bridge:
  ...
  and `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/MainActivity.kt`
...
- Design authority:
  ...
  token source is `lib/core/theme/winflowz_theme_tokens.dart`;
...
- [ ] Task 7: Add Android channel handling and native storage.
  - File: `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/MainActivity.kt`
...
- [ ] Task 8: Extend native keyboard state and sync profile safely.
  - File: `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/KeyboardStateStore.kt`
...
- [ ] Task 9: Render custom actions in the native IME action bar.
  - File: `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/actions/KeyboardActionBarController.kt`
...
- [ ] Task 10: Execute compatible custom action buttons in the IME.
  - File: `android/app/src/main/kotlin/com/winflowz_app/winflowz_app/ime/WinFlowzKeyboardView.kt`
...
- [ ] Task 11: Update docs and manual verification.
  ...
  - Validate with: ShipFlow metadata lint and `git diff --check`.
...
- Design authority:
  `shipflow_data/technical/design-system-authority.md`; use app tokens and
...
- Data flow:
  ...
  `KeyboardActionBarController` render snapshot -> `WinFlowzKeyboardView`
