...
owner: "Diane"
confidence: high
user_story: "En tant qu'utilisatrice WinFlowz, je veux une page Réglages claire, compacte et cohérente, afin de configurer mon compte, mon clavier, la voix, l'overlay et les diagnostics sans lire du jargon technique ni subir des répétitions visuelles."
risk_level: "medium"
security_impact: "yes"
docs_impact: "yes"
linked_systems:
...
supersedes: []
evidence:
  - "User feedback 2026-06-10: Settings account section repeated Compte WinFlowz and Accès WinFlowz in separate cards with duplicated icon and detached connect button."
...
WinFlowz Settings Page UX Remaster
...
En tant qu'utilisatrice WinFlowz, je veux une page Réglages claire, compacte et cohérente, afin de configurer mon compte, mon clavier, la voix, l'overlay et les diagnostics sans lire du jargon technique ni subir des répétitions visuelles.
...
- Account/cloud state is shown as one coherent account/sync summary with contextual connect/sign-out action and without duplicated "WinFlowz" labels.
...
- Respect the monorepo guardrail: governance artifacts stay under root `shipflow_data/`; Flutter changes stay inside `winflowz_app/`.
- Do not commit unless Diane explicitly asks or a later ShipFlow ship skill requires it.
...
- Browser/manual proof required:
  - Flutter web smoke on `https://app.winflowz.com/` after deployment or local web preview when appropriate, checking narrow and desktop layouts for truncation and repeated titles.
...
- Checklist path:
  - If manual QA becomes broad, create `shipflow_data/workflow/test-checklists/winflowz-settings-page-ux-remaster.md`.
- Exceptions:
  - No local Android build or install proof; forbidden by repository guardrails.

## Dependencies

- `shipflow_data/business/branding.md@1.0.0`: practical, calm, direct voice; no unsupported claims.
- `shipflow_data/workflow/specs/winflowz-app-ui-coherence-localization-cleanup.md@1.0.0`: existing broader UI coherence contract; this spec narrows the Settings-specific remaster.
- `shipflow_data/workflow/specs/settings-driven-design-system.md@1.0.0`: active Settings/theme architecture context; this spec must not break theme persistence or backend-agnostic settings storage.
...
- After implementation is deployed to web, smoke `https://app.winflowz.com/` for Settings layout and console errors.
