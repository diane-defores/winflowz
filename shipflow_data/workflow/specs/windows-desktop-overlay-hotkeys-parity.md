...
evidence:
  ...
  - "User decision 2026-05-30: most WinFlowz concepts should not be Android-only; users will expect near-complete platform parity."
...
En tant qu'utilisateur Windows de WinFlowz, je veux retrouver le concept d'overlay flottant, de raccourcis globaux et d'actions rapides de l'app Android, afin de corriger, dicter, transformer, coller et reutiliser mes textes dans n'importe quelle application desktop sans perdre la base commune Flutter.

Acteur principal: utilisateur Windows de WinFlowz.
...
- L'utilisateur selectionne du texte dans une application Windows et appelle WinFlowz.
...
Resultat observable attendu: une surface WinFlowz desktop apparait rapidement au-dessus du contexte de travail Windows, propose les memes actions produit que les surfaces Flutter communes, et livre le resultat par collage ou fallback clipboard sans promettre d'IME Windows.

# Minimal Behavior Contract

WinFlowz doit viser une parite quasi complete entre plateformes: les workflows
...
selection quand c'est disponible, de lancer les actions WinFlowz, puis de livrer
...
Si deux approches donnent un resultat equivalent, WinFlowz doit eviter de
perturber l'utilisateur et conserver le modele mental commun.

# Success Behavior

- Given WinFlowz est lance sur Windows, when l'utilisateur appuie sur le hotkey global configure, then l'overlay WinFlowz apparait sans ouvrir une page marketing ni perdre le focus de travail plus que necessaire.
- Given du texte est selectionne dans l'application active, when l'overlay s'ouvre, then WinFlowz peut recevoir ce texte par le meilleur chemin disponible ou proposer un fallback clipboard explicite.
...
- Given le delivery automatique est autorise par Windows et l'application cible, when un resultat est pret, then WinFlowz tente de le livrer dans l'application active des la premiere version Windows au lieu de se limiter au copier manuel.
- Given l'application active bloque le collage/injection, when le resultat est pret, then WinFlowz garde le texte visible et copie au clipboard avec un message recuperable.
...
- Si l'overlay window ne peut pas etre creee en always-on-top, WinFlowz doit revenir a une fenetre normale explicite et marquer l'overlay Windows comme degrade.
- Si le presse-papiers est indisponible, verrouille ou modifie par une autre app, WinFlowz doit refuser proprement sans perdre le texte produit.
- Si l'application active ne reprend pas le focus apres delivery, WinFlowz doit laisser le resultat copiable et ne pas boucler sur des tentatives invisibles.
...
Les docs actuelles de WinFlowz decrivent correctement l'IME comme Android-only,
...
est un concept WinFlowz portable, tandis que l'implementation Android actuelle
...
global, fenetre overlay, input via selection/clipboard/manual, action WinFlowz,
...
- Parite quasi complete comme principe produit: les concepts WinFlowz sont
...
- IME Windows. Le clavier systeme WinFlowz reste Android-only.
...
- [ ] Tache 6 : Brancher actions WinFlowz communes
...
2. Implementer le flux Windows complet en premiere vague: hotkey global,
   overlay window, input selection/clipboard/manual, actions WinFlowz, clipboard
